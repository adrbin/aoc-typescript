...
function increaseEnergyLevels(map: number[][]) {
  const flashes = [];
...
function flash(flashes: number[][], map: number[][]) {
  let flashesCount = 0;
...
function resetFlashes(map: number[][]) {
  for (let i = 0; i < map.length; i++) {
    for (let j = 0; j < map[i].length; j++) {
      if (map[i][j] > 9) {
        map[i][j] = 0;
...
function isSynchronized(map: number[][]) {
  for (let i = 0; i < map.length; i++) {
    for (let j = 0; j < map[i].length; j++) {
      if (map[i][j] !== 0) {
        return false;
...
runPuzzles(part1, part2, 2021, 11);
